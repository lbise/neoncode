...
The current `frontends/electron/tests/hub-client-auth.ts` test uses a mock WebSocket, fake clock, and Node Web Crypto to verify that the renderer sends no bearer token, accepts a valid reciprocal hub proof, rejects fake hub proofs/malformed capabilities or replay manifests, enforces authentication/welcome deadlines, ignores late traffic, and strictly normalizes session lifecycle metadata. `frontends/electron/tests/hub-manager.ts` covers endpoint-to-health parsing, already-healthy no-op behavior, non-loopback skip behavior, missing bundled-hub diagnostics, WSL copy/start command construction, token redaction from command lines/environments, and start-failure diagnostics with injected fetch/spawn/timer functions. `frontends/electron/tests/token-loader.ts` covers environment tokens, secure WSL token creation script shape, malformed WSL token failure, and exact 64-hex validation. `frontends/electron/tests/reconnect-policy.ts` uses a fake clock to prove capped backoff, single-timer suppression, cancellation/reset, and bounded attach/start fallback without wall-clock sleeps. `command-catalog.ts` exhaustively validates every stable invocation shape and rejects unknown IDs, extra fields, missing/wrong/oversized/control-character arguments, while checking metadata copy isolation and external eligibility. `command-registry.ts` proves enablement descriptions, bounded disabled results, disabled-handler suppression, successful dispatch, and unexpected handler rejection. DOM-free `command-palette.ts` tests cover multi-term/category/search-term filtering and wrapping/empty navigation; no additional DOM harness dependency is introduced for the UI class. `keybindings.ts` verifies the shared schema-5 physical-key contract, formatting, exact-command merge/unbind/reset semantics, 64-entry bound, strict shapes, concrete workspace/pane invocation validation, unsafe printable/modifier/AltGraph/Ctrl+Alt rejection, protected terminal conventions, and effective conflicts. `keybinding-router.ts` verifies `Ctrl+Shift+P`, effective labels, exact existing defaults, unclaimed Ctrl+C/D/Z/readline keys, AltGraph/Ctrl+Alt pass-through, and repeat suppression. `pane-focus-model.ts` covers focus wrap, workspace memory, and removed-pane fallback. `layout-model.ts` tests cover deterministic grid seeding, depth-first pane/split order, nearest nested ancestor lookup for first/second descendants, directional border selection, safe delta clamping/no-match behavior, immutable tab/pane operations, parent collapse, cross-tab moves, ratio clamping, removed leaves, strict known shapes, UTF-8 titles, duplicate identities, and tab/pane/depth limits. `frontends/electron/tests/session-model.ts` covers incarnation/reset/truncation sequence baselines, while `reconnect-output-soak.ts` runs 100 reconnect checkpoints over 10,000 chunks with duplicate suppression and bounded recent output. `terminal-pane-fake-hub.ts` drives the real pane controller through a scripted boot-A/boot-B race, stale callbacks, cursor attach/start fallback, replacement reset, and malformed manifest close. `frontends/electron/tests/config-store.ts` covers config schema 4→5 and older migrations, strict malformed/future keybinding handling, Settings-only merge behavior that preserves profiles/workspaces and ignores environment-effective values, atomic Settings config/backup writes and injected failures, state migrations, valid layout round trips, malformed/oversized/deep/duplicate/future state, aggregate limits, backup recovery, stale temporary-file cleanup, and unusable missing-primary backups. `./dev check` runs all of these without Electron.
...
Most Electron tests should not require the app window to be foreground. Keep a small number of Playwright keyboard tests against xterm's focused textarea for actual key mapping. Pane layout coverage uses DOM locators and structured command execution: both split shortcuts, two/three positive-size panes, configured cwd, new-pane and depth-first F6 focus, ratio geometry plus relaunch persistence, detach/kill close semantics and collapse/sole-pane guards, and detach/kill followed by restart continuity/replacement. Lifecycle and layout tests keep deterministic readiness markers, repeat the tmux wheel hardening path, and do not use `SendKeys`. The hidden Electron suite checks F6/Shift+F6 pane focus and Alt+1/2 workspace focus against authoritative renderer state, active DOM/ARIA state, and the focused xterm. It also checks the visible Commands and Settings buttons; palette-only Settings reachability; General restart labels; keyboard tab selection; exact shortcut recording; inline conflict; recorder Escape; Unbind/Reset; one-shot save; live custom execution; dialog Escape/focus restoration; relaunch persistence; `Ctrl+Shift+P`; palette keyboard selection; and unchanged terminal Ctrl+C/D/Z/Space/L, copy/paste, and Shift+Insert behavior. These keyboard checks do not replace hub/session lifecycle assertions through the structured API.

### 4. Windows release and OS integration smoke
...
- alpha artifact hash/signature verification;
- Microsoft Defender and SmartScreen release-candidate validation on clean machines.
...
Release verification is documented in [release.md](release.md). `./dev verify-release` checks `SHA256SUMS`, `manifest.json`, Authenticode status when signing is required, and a Defender custom scan when Defender tooling is available. Release gates must not disable Defender or add exclusions; a Defender or SmartScreen block is a release blocker and should be handled through artifact preservation and false-positive/reputation submission.
...
- [x] Add strict pane split/resize/close/lifecycle validators, registry/keybinding contracts, nested directional layout helpers, and hidden-Electron split/geometry/cwd/focus/persistence/collapse/detach/kill/restart coverage while preserving Ctrl+C/D/Z/Space/L and paste.
